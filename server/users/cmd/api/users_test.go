...
func TestResetPassword(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method:  http.MethodPost,
			body:    fmt.Sprintf("{\"token\":\"%s\",\"password\":\"12341234\"}", data.MockPasswordResetToken),
			code:    200,
		},
		{
			name:    "Reset password with incorrect token",
			urlPath: "/v1/users/reset-password",
			method:  http.MethodPost,
			body:    "{\"token\":\"abc\",\"password\":\"12341234\"}",
			code:    400,
		},
		{
			...
			urlPath: "/v1/users/reset-password",
			method:  http.MethodPost,
			body:    fmt.Sprintf("{\"token\":\"%s\",\"password\":\"1111\"}", data.MockPasswordResetToken),
			code:    400,
		},
	}
